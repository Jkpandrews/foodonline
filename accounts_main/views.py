...
def registerUser(request):
    if request.user.is_authenticated:
        ...
        if form.is_valid():
            ...
            #Verification
            mail_subject = 'Account verification. Please activate your account'
...
def registerVendor(request):
    if request.user.is_authenticated:
        ...
        if form.is_valid() and v_form.is_valid():
            ...
            messages.success(request,'your Restaurant account has been created successfully!')
...
def reset_password_validate(request,uidb64,token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        # messages.success('please reset your password')
...
def forgot_password(request):
    if request.method == 'POST':
        ...
        if User.objects.filter(email=email).exists():
            ...
            messages.success(request, 'Password rest link sent successful!')
